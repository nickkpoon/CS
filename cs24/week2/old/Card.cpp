...
deck *make_standard_deck()
{
	...
	int i;
	deck* newdeck[i];
	newdeck[i]->num_cards = 52;
	
	for (i = 1; i < 13; i++)
	{
		newdeck[i*1] = malloc(sizeof(card));
		newdeck[i*2] = malloc(sizeof(card));
		newdeck[i*3] = malloc(sizeof(card));
		newdeck[i*4] = malloc(sizeof(card));
		newdeck[i*1]->cards = make_card(0, i);
		newdeck[i*2]->cards = make_card(1, i*2);
		newdeck[i*3]->cards = make_card(2, i*3);
		newdeck[i*4]->cards = make_card(3, i*4);
	}
	
	//for (i = 1; i < 52; i++)
	//{
	//	newdeck[i] = malloc(sizeof(card));

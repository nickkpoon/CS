...
public class Polynomial extends ArrayList<Integer> {
	...
	if (this.getDegree() < 0)
		...
		return valueof(this.get(0));
	
	//-------------------Finished the 0 degree polynomial---------------------
    ...
    */
	...
	for (int i = 0; i < length; i++)
	{
		if (i == length -1 && coeffsHighToLow[i] == 0){
			return zero;
		}
	...
	int size = length - stop;
	int [] answer = new int[size];

	for (int j = 0; j < size; j++)
	{
		answer[j] = coeffsHighToLow[length-1-j];
	}
    ...
    */
	...
	for (int j = 0; j < size; j++)
	{
		answer[j] = coeffsLowToHigh[stop-j];
	}
